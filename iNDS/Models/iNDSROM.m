...
+ (NSArray*)ROMsAtPath:(NSString*)gamesPath saveStateDirectoryPath:(NSString*)saveStatePath
{
    ...
    for(NSString *file in files) {
        if ([file.pathExtension isEqualToString:@"nds"]) {
            iNDSROM *game = [iNDSROM ROMWithPath:[gamesPath stringByAppendingPathComponent:file] saveStateDirectoryPath:saveStatePath];
...
{
    return [[iNDSROM alloc] initWithPath:path saveStateDirectoryPath:saveStatePath];
}

- (iNDSROM*)initWithPath:(NSString*)path saveStateDirectoryPath:(NSString*)saveStatePath
{
    NSAssert(path.isAbsolutePath, @"iNDSROM path must be absolute");
...
- (NSString*)gameTitle
{
    ...
    if (title == nil) {
        ...
        uint32_t titleOffset = 0x240 + 0x100 * [iNDSROM preferredLanguage];
...
{
    return [NSString stringWithFormat:@"<iNDSROM 0x%p: %@>", self, self.path];
}
