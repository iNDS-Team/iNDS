...
- (void)drawSnapLineAtPosition:(NSInteger)positon Direction:(NSInteger)direction
{
    ...
    if (direction == VERTICAL)
        snapLine.frame = CGRectMake(positon, 0, 1, self.indicatorView.frame.size.height);
    else
        snapLine.frame = CGRectMake(0, positon, self.indicatorView.frame.size.width,  1);
    snapLine.zPosition = 100;
    snapLine.name = @"Snap";
    
    [self.indicatorView.layer addSublayer:snapLine];
}
...
{
    return [[[[iNDSSettings sharedInstance] batteryPath] stringByAppendingPathComponent:name] stringByAppendingPathExtension:@"ips"];
}
